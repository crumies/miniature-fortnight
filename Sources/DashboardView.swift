        ...
        GlassCard {
            VStack(spacing: 12) {
                ...
                HStack {
                    ...
                    metric("Power", String(format: "%.1f kW", ble.telemetry.powerKw))
                }
                HStack {
                    metric("Battery", String(format: "%.0f %%", ble.telemetry.batteryPercent))
                    metric("Pack", "72V 38.4Ah")
                }
            }
        }
    }
...
struct RPMArc: View {
    ...
    var body: some View {
        ZStack {
            ...
            Circle()
                .trim(from: startTrim, to: startTrim + min(progress, redStart) * totalTrim)
                .stroke(baseColor, style: StrokeStyle(lineWidth: 15, lineCap: .round))
                .rotationEffect(.degrees(90))
                .shadow(color: baseColor.opacity(0.35), radius: 10)

            if progress > redStart {
                Circle()
                    .trim(from: startTrim + redStart * totalTrim, to: startTrim + progress * totalTrim)
                    .stroke(.red, style: StrokeStyle(lineWidth: 15, lineCap: .round))
                    .rotationEffect(.degrees(90))
                    .shadow(color: .red.opacity(0.50), radius: 12)
            }
        }
    }
}
...
struct HUDAddMenu: View {
    ...
    var body: some View {
        Menu {
            ...
            Toggle("Lean meter", isOn: $settings.hudShowLean)
